...
confirmation_phrase="RESET EURIPUS PROD"
...
wait_for_server_health() {
  ...
  while (( SECONDS < deadline )); do
    ...
    if [[ "$server_status" == *"exited"* || "$server_status" == *"unhealthy"* ]]; then
      ...
      echo "Server failed to become healthy during production reset." >&2
      echo >&2
      echo "Server logs:" >&2
      echo "$logs" >&2
      exit 1
  ...
  local logs
  logs="$(get_server_logs)"
  echo "Server did not become healthy within $timeout_seconds seconds during production reset." >&2
  echo >&2
  echo "Server logs:" >&2
  echo "$logs" >&2
  return 1
...
if [[ "$confirmation_input" != "$confirmation_phrase" ]]; then
  echo "Confirmation did not match. Aborting production reset." >&2
  exit 1
fi
...
echo
echo "Euripus production reset complete."
