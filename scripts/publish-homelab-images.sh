...
platform="${EURIPUS_PUBLISH_PLATFORM:-$platform}"
moving_tag="${EURIPUS_IMAGE_TAG:-$moving_tag}"
server_image="${EURIPUS_SERVER_IMAGE:-$server_image}"
web_image="${EURIPUS_WEB_IMAGE:-$web_image}"

sha_tag="$(get_git_sha)"

invoke_ghcr_login_if_configured

publish_image "$server_image" "apps/server/Dockerfile" "$sha_tag" "$moving_tag"
