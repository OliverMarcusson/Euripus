...
export function OnDemandPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <TitleDialog
        titleId={selected?.id ?? null}
        mediaType={selected?.mediaType ?? null}
        fallback={selected}
        onOpenChange={(open) => { if (!open) setSelected(null); }}
        onFavorite={(title) => titleFavorite.mutate(title)}
      />
    </div>
  );
}
...
function TitleCard({ title, onOpen, onFavorite }: { title: OnDemandTitle; onOpen: () => void; onFavorite: () => void }) {
  return <div className="group relative overflow-hidden rounded-xl border border-border/50 bg-card transition hover:border-primary/50">
    <button className="block w-full text-left" onClick={onOpen}>
      ...
      <div className="p-3"><p className="line-clamp-2 font-medium">{title.name}</p><div className="mt-1 flex items-baseline justify-between gap-2 text-xs text-muted-foreground"><span className="truncate">{title.releaseDate ?? title.categoryName ?? ""}</span><span className="max-w-[55%] shrink-0 truncate rounded bg-muted px-1.5 py-0.5 font-medium">{title.providerLabel}</span></div></div>
    </button>
